...
<div class="block-flat">
    <h1 class="page-header">Urejanje novice: <?= $news['title'] ?></h1>

    <form action="editingNews.php?id=<?= $news["id"]; ?>" method="POST" class="ajaxForm">
        <div class="form-group">
            <label for="title">Naslov:</label>
            <input type="text" id="title" name="title" class="form-control" value="<?= $news['title'] ?>" required placeholder="Naslov">
        </div>
        <div class="form-group">
            <label for="textarea">Vsebina:</label>
            <textarea name="text" class="form-control" placeholder="Vsebina" id="textarea"><?= $news["text"] ?></textarea>
        </div>
        <div class="form-group">
            <input type="submit" class="btn btn-success" value="Uredi novico"/>
        </div>
    </form>
